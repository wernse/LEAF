...
class SubnetAlexNet_norm(nn.Module):
    def __init__(self, taskcla, sparsity=0.5):
        ...
        self.taskcla = taskcla
        self.last = nn.ModuleList()
        self.adapt_last = nn.ModuleList()
        for t, n in self.taskcla:
            self.last.append(nn.Linear(2048, n, bias=False))

        for t, n in self.taskcla:
            self.adapt_last.append(TALE(n))
...
class TALE(nn.Module):
    def __init__(self, classes):
        ...
        self.adapt_fc2 = SubnetLinear(2048, 2048, sparsity=1, bias=False)
        self.adapt_bn2 = nn.BatchNorm1d(2048, track_running_stats=False, affine=False)
        self.adapt_last = nn.Linear(2048, classes, bias=False)


    def forward(self, x):
        # Set for second mas
        x = self.adapt_fc1(x, weight_mask=None, bias_mask=None, mode='joint')
        x = self.drop2(self.relu(self.adapt_bn1(x)))
        x = self.adapt_fc2(x, weight_mask=None, bias_mask=None, mode='joint')
        x = self.drop2(self.relu(self.adapt_bn2(x)))
        y = self.adapt_last(x)
        return y
