...
def get(data_path, args, seed=0, pc_valid=0.10, samples_per_task=-1, task_order=None):
    ...
    for t in data.keys():
        ...
        X = data[t]['train']['x']
        y = data[t]['train']['y']
        print(len(data[t]['train']['y']))
        # Initialize the StratifiedShuffleSplit object
        sss = StratifiedShuffleSplit(n_splits=1, test_size=pc_valid, random_state=seed)
        # Get index arrays for splitting
        for train_index, valid_index in sss.split(X, y):
            itrain, ivalid = train_index, valid_index

        # Use these indices to create stratified training and validation sets
        itrain = torch.LongTensor(itrain)
        ivalid = torch.LongTensor(ivalid)
        ...
        counter = Counter([x.item() for x in data[t]['valid']['y']])
        print(counter)
